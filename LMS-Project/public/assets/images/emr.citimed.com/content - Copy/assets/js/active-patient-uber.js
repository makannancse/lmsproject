...
target.innerHTML = rows.map(function (r) {
  return '<article class="active-patient-card" data-patient-id="' + escapeHtml(r.patient_id) + '">' +
    ...
    '<div class="active-card-col pickup-col">' +
      '<button class="ghost-btn request-pickup-btn" type="button" data-patient-id="' + escapeHtml(r.patient_id) + '">' +
        'Request Pickup' +
      '</button>' +
    '</div>' +
  ...
  target.querySelectorAll('.request-pickup-btn').forEach(function (btn) {
    btn.onclick = function (e) {
      e.stopPropagation();
      openPickupModal(btn.getAttribute('data-patient-id'));
    };
  });
...
function bindPickupModal() {
  var closeBtn = document.getElementById('closePickupModalBtn');
  var closeXBtn = document.getElementById('closePickupXBtn');
  var submitBtn = document.getElementById('submitPickupBtn');
  var modal = document.getElementById('pickupModal');

  if (closeBtn) closeBtn.onclick = closePickupModal;
  if (closeXBtn) closeXBtn.onclick = closePickupModal;
  if (submitBtn) submitBtn.onclick = submitPickupRequest;

  if (modal) {
    modal.addEventListener('click', function (e) {
      if (e.target === modal) closePickupModal();
    });
  }
}

function openPickupModal(patientId) {
  var row = activeRows.find(function (r) {
    return String(r.patient_id) === String(patientId);
  }) || {};

  document.getElementById('pickupPatientId').value = patientId || '';
  document.getElementById('pickupPatientLabel').textContent = row.patient_full_name
    ? 'Pickup for ' + row.patient_full_name + ' (#' + patientId + '). Confirm details before sending.'
    : 'Confirm pickup details before sending the Uber Health request.';
  document.getElementById('pickupPhone').value = row.cell_phone_nbr || '';
  document.getElementById('pickupAddress').value = row.pickup_location || '';
  document.getElementById('dropoffAddress').value = row.facility_desc || '';
  document.getElementById('pickupNotes').value = '';
  document.getElementById('pickupStatus').textContent = '';

  setDefaultPickupTime(row.next_appointment_date || '');
  document.getElementById('pickupModal').hidden = false;
}
...
function setDefaultPickupTime(nextAppointmentDate) {
  ...
  var parts = String(nextAppointmentDate).split('/');
  if (parts.length !== 3) {
    input.value = '';
    return;
  }
...
async function submitPickupRequest() {
  var status = document.getElementById('pickupStatus');
  var submitBtn = document.getElementById('submitPickupBtn');

  var payload = {
    patient_id: document.getElementById('pickupPatientId').value,
  ...
  if (!payload.patient_id || !payload.pickup_address || !payload.dropoff_address || !payload.pickup_time) {
    status.textContent = 'Patient, pickup address, dropoff address, and pickup time are required.';
    return;
  }

  status.textContent = 'Requesting pickup...';
  if (submitBtn) submitBtn.disabled = true;

  try {
    var res = await fetch('/new-ui/content/api/request-uber-health-pickup.php', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      credentials: 'include',
    ...
    if (!res.ok || !json.ok) {
      throw new Error(json.message || 'Unable to request pickup.');
    }

    status.textContent = (json.mock ? 'Mock pickup created. ' : 'Pickup requested. ') + 'Ride ID: ' + (json.ride_id || 'pending');
  } catch (err) {
